...
module.exports = {
  ...
  findStation: function (req, res) {
    ...
    Data.find({ direccion_estacion: direccion_estacion }).limit(limiteDefecto)
      .then(_data => {
  ...
  findStationType: function (req, res) {
    ...
    Data.find({ direccion_estacion: direccion_estacion ,tipo_sensor:tipo_sensor}).limit(limiteDefecto)
      .then(_data => {
        if (!_data) return res.notFound({ err: 'No se encontró información' });
        return res.ok(_data);
      })
  ...
  findStationTypeLimit: function (req, res) {
    ...
    Data.find({ direccion_estacion: direccion_estacion },{tipo_sensor:tipo_sensor}).limit(limite_definido)
      .then(_data => {
        if (!_data) return res.notFound({ err: 'No se encontró información' });
        return res.ok(_data);
      })
